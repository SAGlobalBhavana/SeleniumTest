...
namespace HybridFramework.Reusable_Functions.D365FO
{
    public  class FO_Reusable : TestBase
    {
        ...
        public void ClickLastButton(String ElementXpath)
        {
            TimeWaitsHelper.WaitForClickable(driver, By.XPath(ElementXpath));
            IList<IWebElement> closeButtons = driver.FindElements(By.XPath(ElementXpath));
            int count = closeButtons.Count;
            Console.WriteLine("Last Button count = " + count);
            closeButtons[count - 1].Click();
            Thread.Sleep(2000);
        }
